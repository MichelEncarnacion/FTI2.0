...
import { FaArrowLeft, FaChartLine, FaHandshake, FaBriefcase } from 'react-icons/fa';
...
export default function SapsTrabajoCompetitividad() {
  ...
  // === 2. FETCH PROYECTOS TRABAJO Y COMPETITIVIDAD ===
  useEffect(() => {
    const fetchProyectos = async () => {
      ...
      try {
        const { data, error } = await supabase
          ...
          `)
          .eq('categoria', 'trabajo-competitividad')
          .eq('estado', 'activo');

        if (error) throw error;
        setProyectos(data || []);
      } catch (error) {
        console.error("Error en Trabajo y Competitividad:", error.message);
        setProyectos([]);
      } finally {
        setLoading(false);
      }
    ...
    <div className="min-h-screen bg-[#050507] text-gray-200 pb-20">
      ...
      <section className="max-w-7xl mx-auto px-6">
        <div className="relative rounded-3xl overflow-hidden border border-white/5 bg-gradient-to-br from-[#0f1116] to-[#050507] shadow-2xl">
          <div className="absolute top-0 right-0 w-1/2 h-full bg-emerald-600/5 blur-[100px] -z-0" />

          <div className="grid grid-cols-1 md:grid-cols-12 relative z-10">
            <div className="md:col-span-8 p-8 md:p-12 space-y-4">
              <div className="flex items-center gap-3 mb-2">
                <span className="bg-emerald-600 text-[10px] font-black px-3 py-1 rounded-full uppercase tracking-tighter text-white">SAPS</span>
                <span className="text-emerald-500 text-[10px] font-black uppercase tracking-widest flex items-center gap-2">
                  <FaChartLine /> Crecimiento Sostenible
                </span>
              </div>
              <h1 className="text-4xl md:text-5xl font-black text-white tracking-tighter leading-tight">
                Trabajo y <br /> <span className="text-emerald-500">Competitividad</span>
              </h1>
              <p className="max-w-xl text-gray-400 text-sm md:text-base font-light leading-relaxed">
                Modelos que articulan ética empresarial e inclusión económica para formar líderes capaces de generar empleo digno y bienestar social.
              </p>
            </div>

            <div className="md:col-span-4 bg-emerald-600/10 flex items-center justify-center p-8 border-l border-white/5">
              <div className="relative group">
                <div className="absolute inset-0 bg-emerald-500/20 blur-xl group-hover:blur-3xl transition-all" />
                <img src={dinero} alt="Icono Trabajo" className="relative h-32 w-32 object-contain drop-shadow-2xl" />
              </div>
            </div>
          </div>
        </div>
      ...
      <section className="max-w-7xl mx-auto px-6 mt-16">
        <div className="flex items-center justify-between mb-10">
          <h2 className="text-xl font-bold text-white flex items-center gap-3">
            <span className="h-2 w-2 bg-emerald-500 rounded-full shadow-[0_0_10px_#10b981]" />
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-24 border border-dashed border-white/10 rounded-3xl bg-[#0a0a0f]">
            <FaHandshake className="text-gray-700 text-5xl mb-4" />
            <p className="text-gray-400 font-medium tracking-tight">Próximamente nuevos proyectos.</p>
          </div>
        ) : (
          <div className="grid gap-8 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            {proyectos.map((p) => (
              <div
                ...
                className="group relative flex flex-col bg-[#0f1116] rounded-2xl border border-white/5 hover:border-emerald-500/30 transition-all duration-500 shadow-xl cursor-pointer hover:-translate-y-2 overflow-hidden"
              >
                <div className="absolute top-4 right-4 z-20 bg-black/60 backdrop-blur-md border border-white/10 px-2 py-1 rounded-md">
                  <p className="text-[9px] font-black text-emerald-400 uppercase tracking-widest">{p.grado || 'Gestión'}</p>
                </div>
                ...
                <div className="h-44 w-full bg-black overflow-hidden relative">
                  {p.imagen_destacada_path ? (
                    ...
                    <div className="w-full h-full flex items-center justify-center bg-[#0a0a0f]">
                      <FaBriefcase className="text-gray-800 text-3xl" />
                    </div>
                  )}
                ...
                <div className="p-6 flex flex-col flex-grow space-y-4">
                  <div className="flex items-center gap-3">
                    <img
                      ...
                      className="h-8 w-8 rounded-full object-cover ring-2 ring-emerald-500/20 group-hover:ring-emerald-500 transition-all"
                      alt="Prof"
                    />
                    <p className="text-[11px] font-bold text-gray-300 truncate tracking-tight">{p.profesor?.nombre || 'Docente'}</p>
                  </div>

                  <h3 className="text-base font-black text-white leading-tight group-hover:text-emerald-400 transition-colors line-clamp-2">
                    {p.titulo}
                  </h3>

                  <div className="flex flex-wrap gap-2 pt-2">
                    {p.tecnologias?.split(',').slice(0, 2).map((tag, i) => (
                      <span key={i} className="text-[9px] font-black uppercase tracking-tighter bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 px-2 py-1 rounded-md">
                        {tag.trim()}
                      </span>
                    ))}
                  </div>
                ...
                <div className="px-6 pb-6">
                  <div className="flex items-center justify-between text-[10px] font-bold text-gray-500 group-hover:text-emerald-500 transition-colors">
